import os
import json
from blockchain_demo.wallet import generate_wallet, sign
...
def main():
    ...
    for idx, name in enumerate(["Val-A", "Val-B", "Val-C"], start=1):
        ...
        sigs[wallet_data["public_key"]] = sign(wallet_data, 'genesis')
    ...
    genesis_block = Block(header=header, transactions=[], state=state, balances=balances, finalized=True, validator_signatures=sigs, signers_frozen=sorted(list(sigs.keys())))
