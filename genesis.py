...
def main():
    ...
    genesis_block = Block(header=header, transactions=[], state=state, balances=balances, finalized=True)
    genesis_json = genesis_block.to_json()
    genesis_json["hash"] = '0'*64
    genesis_json["validator_signatures"] = {}
    genesis_json["signers_frozen"] = []
